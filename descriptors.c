...
static const USB_Descriptor_HIDReport_Datatype_t PROGMEM usb_report_desc[] = {
    ...
    HID_RI_COLLECTION(8, 1), // application
        ...
        HID_RI_LOGICAL_MINIMUM(8, 0),
        HID_RI_LOGICAL_MAXIMUM(8, 0xff), // commercial keyboard reports 255, so we do too
        HID_RI_USAGE_PAGE(8, 7), // key codes
        HID_RI_USAGE_MINIMUM(8, 0),
        HID_RI_USAGE_MAXIMUM(8, 0xff), // commercial keyboard reports 255, so we do too
        HID_RI_INPUT(8, HID_IOF_DATA | HID_IOF_ARRAY), // keys are reported as an array of keys which are held down; 0x00 indicates the array entry is not used
...
static const USB_Descriptor_Device_t PROGMEM usb_device_desc = {
	...
	.ReleaseNumber          = VERSION_BCD(0,0,1), // ReleaseNumber can be anything you want. LUFA suggests using this field to destinguish different projects
...
static const struct {
    ...
    USB_Descriptor_Endpoint_t             hid_report;
	...
	.hid_keyboard =
		{
			...
			.CountryCode            = 0x00, // commercial keyboards have CC = 0 also, so it seems there is no need to fill this in
	...
	.hid_report = {
			...
			.Attributes             = EP_TYPE_INTERRUPT | ENDPOINT_ATTR_NO_SYNC | ENDPOINT_USAGE_DATA, // we are a plain interrupt endpoint
    ...
    switch (val>>8) {
        ...
        case DTYPE_String:
            ...
            switch (val & 0xff) {
                case 0:
                    d = &usb_language_str;
                    s = pgm_read_byte(&usb_language_str.Header.Size);
                    break;
                case 1:
                    d = &usb_manufacturer_str;
                    s = pgm_read_byte(&usb_manufacturer_str.Header.Size);
                    break;
                case 2:
                    d = &usb_product_str;
                    s = pgm_read_byte(&usb_product_str.Header.Size);
                    break;
            }
            break;
        case HID_DTYPE_HID:
